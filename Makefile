...
PROJECT_NAME=markiezenhof
STACK_NAME=www-markiezenhof-nl
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://markiezenhof.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t github.com/intk/markiezenhof-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t github.com/intk/markiezenhof-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name markiezenhof-backend-acceptance -d github.com/intk/markiezenhof-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name markiezenhof-frontend-acceptance --link markiezenhof-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d github.com/intk/markiezenhof-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop markiezenhof-frontend-acceptance
	@docker stop markiezenhof-backend-acceptance
